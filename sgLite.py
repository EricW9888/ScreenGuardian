...
class ScreenGuardian(QMainWindow):
    def __init__(self):
        ...
        self.tracking_on_button = QPushButton("Enable Focus Monitoring", self)
    ...
    def tutorial_screen(self):
        webbrowser.open("https://sg-web-docs.pages.dev/")
        self.start_tutorial = False
        os.remove(self.settings_file_name)
        with open(self.settings_file_name, "w") as self.file:
            self.update_settings()
    ...
    def update_frame(self):
        ...
        if (time.time() - self.last_time) >= 0.25:
            ...
            self.left_eye = [(self.shape.part(36).x, self.shape.part(36).y), (self.shape.part(37).x, self.shape.part(37).y), (self.shape.part(38).x, self.shape.part(38).y), (self.shape.part(39).x, self.shape.part(39).y), (self.shape.part(40).x, self.shape.part(40).y), (self.shape.part(41).x, self.shape.part(41).y)]
            self.right_eye = [(self.shape.part(42).x, self.shape.part(42).y), (self.shape.part(43).x, self.shape.part(43).y), (self.shape.part(44).x, self.shape.part(44).y), (self.shape.part(45).x, self.shape.part(45).y), (self.shape.part(46).x, self.shape.part(46).y), (self.shape.part(47).x, self.shape.part(47).y)]

            #Calculate average eye aspect ratio
            left_ear = self.calculate_eye_aspect_ratio(self.left_eye)
            right_ear = self.calculate_eye_aspect_ratio(self.right_eye)
        ...
        left_eye_outer = (self.shape.part(36).x, self.shape.part(36).y)
        right_eye_outer = (self.shape.part(45).x, self.shape.part(45).y)
        ...
        for (x, y) in self.left_eye:
            cv2.circle(self.frame, (x, y), 2, (0, 255, 0), -1)
        for (x, y) in self.right_eye:
            cv2.circle(self.frame, (x, y), 2, (0, 255, 0), -1)
